...
namespace fail2ban_win.utils
{
    internal class SQLiteHelper
    {
        ...
        public string GetSQLiteVersion()
        {
            ...
            {
                dbCommand = dbConnection.CreateCommand();
                dbCommand.CommandText = "SELECT SQLITE_VERSION()";
        ...
        public string ExecuteQueryForString(string queryString)
        {
            try
            {
                dbCommand = dbConnection.CreateCommand();
        ...
        public SQLiteDataReader ReadFullTable(string tableName, int count = 0)
        {
            string queryString = "SELECT * FROM " + tableName;
            if (count > 0) queryString += " limit " + count;
        ...
        public int CreateTable(string tableName, string[] colNames, string[] colTypes, int primaryKey = -1)
        {
            string queryString = "CREATE TABLE IF NOT EXISTS " + tableName + "( ";
            for (int i = 0; i < colNames.Length; i++)
            {
                if (i == 0) queryString += colNames[i] + " " + colTypes[i];
                else queryString += ", " + colNames[i] + " " + colTypes[i];
                if (i == primaryKey) queryString += " PRIMARY KEY ";
            }
            queryString += " ) ";
            return ExecuteNonQuery(queryString);
        }
